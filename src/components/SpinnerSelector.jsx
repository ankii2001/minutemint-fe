...
export default function SpinnerSelector({ names }) {
  ...
  useEffect(() => {
    ...
    names.forEach((name, i) => {
      ...
      // 2) Draw rotated, truncated / wrapped label
      ctx.save();
      ctx.translate(center, center);
      const bisector = start + slice / 2;
      ctx.rotate(bisector);
      ...
      // Break into lines or truncate
      let lines;
      if (measure(name) <= maxW) {
        lines = [name];
      } else {
        ...
        if (measure(first) <= maxW && measure(second) <= maxW) {
          lines = [first, second];
        } else {
          // Fallback truncate
          let txt = name;
          while (measure(txt + "...") > maxW && txt.length) {
            txt = txt.slice(0, -1);
          }
          lines = [txt + "..."];
        }
      ...
      // Draw each line centered on the radius direction
      const lineH = 14;
      const totalH = lines.length * lineH;
      lines.forEach((ln, idx) => {
        const y = -totalH / 2 + idx * lineH + lineH / 2;
        ctx.fillText(ln, radius - 20, y);
      });
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      {winner && !spinning && (
        <div
          ...
          text-xl md:text-4xl font-extrabold text-green-700
          animate-bounce
        "
        >
          <span>🎉</span>
